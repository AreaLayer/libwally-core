 ...
 * :param abf: Asset Blinding Factor (Random entropy to blind with). Must be non-NULL when ``asset`` is a commitment.
...
/**
 * Generate a rangeproof nonce hash using SHA256(ECDH(pub_key, priv_key).
 *
...
/**
 * Generate an explicit value rangeproof for a transaction output.
 *
